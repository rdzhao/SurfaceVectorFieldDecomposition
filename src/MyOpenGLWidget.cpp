...
void OGLWidget::setVectorFieldModule(RenderModule* rm)
{
	...
	for (Facet_iterator fi = mesh->facets_begin(); fi != mesh->facets_end(); ++fi)
	{
		v.push_back(mesh->barycenter_en()[fi].x() - mesh->xcenter());
		v.push_back(mesh->barycenter_en()[fi].y() - mesh->ycenter());
		v.push_back(mesh->barycenter_en()[fi].z() - mesh->zcenter());
		//v.push_back(mesh->barycenter_en()[fi].x() - mesh->xcenter() + mesh->coexactVF()[fi].x()*0.04);
		//v.push_back(mesh->barycenter_en()[fi].y() - mesh->ycenter() + mesh->coexactVF()[fi].y()*0.04);
		//v.push_back(mesh->barycenter_en()[fi].z() - mesh->zcenter() + mesh->coexactVF()[fi].z()*0.04);
		v.push_back(mesh->barycenter_en()[fi].x() - mesh->xcenter() + mesh->originalVF()[fi->idx()].x()*0.04);
		v.push_back(mesh->barycenter_en()[fi].y() - mesh->ycenter() + mesh->originalVF()[fi->idx()].y()*0.04);
		v.push_back(mesh->barycenter_en()[fi].z() - mesh->zcenter() + mesh->originalVF()[fi->idx()].z()*0.04);

		n.push_back(fi->normal().x());
		n.push_back(fi->normal().y());
		n.push_back(fi->normal().z());
		n.push_back(fi->normal().x());
		n.push_back(fi->normal().y());
		n.push_back(fi->normal().z());
...
void OGLWidget::selectEdge(int wx, int wy)
{
	...
	if (intersected)
	{
		...
		cout << "Edge Idx: " << eIdx / 2 << ", 1 Form: " << mesh->oneForm()[mesh->halfedge(eIdx)] << endl;
		update();
	}
}
...
void OGLWidget::decomposition()
{
	//mesh->random1Form();
	mesh->readVectorField();
	mesh->convertTo1Form();

	mesh->computeElementVolume();
	mesh->computeBarycenter();

	mesh->buildHodgeStar();
	mesh->buildExteriorDerivative();
	
	mesh->compute0FormPotential();
	mesh->compute2FormPotential();
	mesh->computeHarmonic();

	mesh->computeDecomposedVectorField();

	// add render module for vector field
