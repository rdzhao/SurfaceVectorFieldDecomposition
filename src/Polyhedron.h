...
private:
	...
	// input
	std::vector<Vector> vectorField;

	// decomposition
	std::map<Facet_iterator, Vector> center;
	std::map<Edge_iterator, double> _1Form;
	std::map<Vertex_iterator, double> alpha0Form;
	std::map<Facet_iterator, double> beta2Form;
	std::map<Edge_iterator, double> exact1Form;
	std::map<Edge_iterator, double> coexact1Form;
	std::map<Edge_iterator, double> harmonic1Form;

	std::map<Facet_iterator, Vector> exactVectorField;
	std::map<Facet_iterator, Vector> coexactVectorField;
	std::map<Facet_iterator, Vector> harmonicVectorField;

	std::map<Vertex_iterator, double> vertexDualVol;
	std::map<Edge_iterator, double> edgePrimalVol;
	std::map<Edge_iterator, double> edgeDualVol;
	std::map<Facet_iterator, double> facetPrimalVol;

	Eigen::SparseMatrix<double> hodgeStar0Form;
	Eigen::SparseMatrix<double> hodgeStar1Form;
	Eigen::SparseMatrix<double> hodgeStar2Form;

	Eigen::SparseMatrix<double> exteriorDerivative0Form;
	Eigen::SparseMatrix<double> exteriorDerivative1Form;

public:
	void random1Form();
	void readVectorField();
	void convertTo1Form();
	...
	void buildHodgeStar();
	void buildExteriorDerivative();
	
	void compute0FormPotential();
	void compute2FormPotential();
	void computeHarmonic();

	void computeDecomposedVectorField();

	std::vector<Vector>& originalVF() { return vectorField; }
	std::map<Edge_iterator, double>& oneForm() { return _1Form; }
	std::map<Facet_iterator, Vector>& exactVF() { return exactVectorField; }
	std::map<Facet_iterator, Vector>& coexactVF() { return coexactVectorField; }
	std::map<Facet_iterator, Vector>& harmonicVF() { return harmonicVectorField; }
	std::map<Facet_iterator, Vector>& barycenter_en() { return center; }
...
public:
	...
	// return cotan for the opposite corner
	FT cotangent(Halfedge_handle he)
	{
		Vector pa = he->vertex()->point() - he->next()->vertex()->point();
		Vector pb = he->prev()->vertex()->point() - he->next()->vertex()->point();
		// (pa . pb)/((pa x pb).len)
		FT dot = pa * pb;
