...
void Viewer::createMenu()
{
	...
	sceneMenu->setFixedWidth(250);
	{
		...
		showExactAct = new QAction("Show Exact Field", this);
		showExactAct->setShortcut(QKeySequence(tr("Ctrl+E")));
		sceneMenu->addAction(showExactAct);

		showCoexactAct = new QAction("Show Coexact Field", this);
		showCoexactAct->setShortcut(QKeySequence(tr("Ctrl+C")));
		sceneMenu->addAction(showCoexactAct);

		showHarmonicAct = new QAction("Show Harmonic Field", this);
		showHarmonicAct->setShortcut(QKeySequence(tr("Ctrl+H")));
		sceneMenu->addAction(showHarmonicAct);
	...
	{
		decomAct = new QAction("Vector Field Decomposition", this);
		decomAct->setShortcut(tr("Crtl+O"));
		opMenu->addAction(decomAct);
		connect(decomAct, SIGNAL(triggered()), this, SLOT(decompisitionSlot()));
	}
}
...
{
	canvas->setVectorFieldVisible(true);
}
